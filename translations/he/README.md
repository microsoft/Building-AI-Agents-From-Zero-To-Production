...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-24T23:35:58+00:00",
...
[![בקשות משיכה של GitHub](https://img.shields.io/github/issues-pr/microsoft/Building-AI-Agents-From-Zero-To-Production.svg)](https://GitHub.com/microsoft/Building-AI-Agents-From-Zero-To-Production/pulls/?WT.mc_id=academic-105485-koreyst)
[![בקשות משיכה מתקבלות בברכה](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat-square)](http://makeapullrequest.com?WT.mc_id=academic-105485-koreyst)

[![Discord של Microsoft Foundry](https://dcbadge.limes.pink/api/server/nTYy5BXMWG)](https://discord.gg/Kuaw3ktsu6)

## 🌱 איך להתחיל

הקורס הזה כולל שיעורים המכסים את היסודות של בנייה ופריסה של סוכני בינה מלאכותית.

כל שיעור בונה על הקודם, לכן אנו ממליצים להתחיל מההתחלה ולעבוד עד הסוף.

אם אתם רוצים לחקור עוד נושאים על סוכני AI, תוכלו לבדוק את [קורס סוכני AI למתחילים](https://aka.ms/ai-agents-beginners).

### פגשו לומדים אחרים וקבלו תשובות לשאלותיכם

אם תיתקלו בבעיות או יהיו לכם שאלות לגבי בניית סוכני AI, הצטרפו לערוץ הייעודי שלנו ב-[Discord של Microsoft Foundry](https://discord.gg/Kuaw3ktsu6).

### מה צריך

לכל שיעור יש דוגמת קוד משל עצמה שאפשר להריץ מקומית. תוכלו [לעשות fork למאגר זה](https://github.com/microsoft/Building-AI-Agents-From-Zero-To-Production/fork) כדי ליצור עותק משלכם.

הקורס הזה כרגע משתמש ב:
...
אנא ודאו שיש לכם גישה לשירותים האלה לפני תחילת העבודה.

אפשרויות נוספות בנושא אירוח מודלים ושירותים מגיעות בקרוב. 

## 🗃️ שיעורים

| **שיעור**         | **תיאור**                                                                                  |
|--------------------|--------------------------------------------------------------------------------------------------|
| [Agent Design](./lesson-1-agent-design/README.md)       | מבוא למקרה השימוש שלנו "קליטת מפתחים" ואיך לעצב סוכנים יעילים  |
| [Agent Development](./lesson-2-agent-development/README.md)  | שימוש ב-Microsoft Agent Framework (MAF) — צרו 3 סוכנים כדי לעזור למפתחים חדשים להשתלב.       |
| [Agent Evaluations](./lesson-3-agent-evals/README.md)  | שימוש ב-Microsoft Foundry כדי לבדוק עד כמה הסוכנים שלנו מתפקדים וכיצד לשפר אותם. |
| [Agent Deployment](./lesson-4-agent-deployment/README.md)   | שימוש ב-Hosted Agents וב-OpenAI Chatkit — ראו כיצד לפרוס סוכן בינה מלאכותית לסביבת ייצור.       |
...
פרויקט זה מקבל בברכה תרומות והצעות. רוב התרומות מחייבות שתסכים להסכם רישיון תורם (CLA) המצהיר שיש לך את הזכות, ושאתה אכן מעניק לנו את הזכויות להשתמש בתרומתך. לפרטים, בקר ב-<https://cla.opensource.microsoft.com>.

כאשר תגיש בקשת משיכה, רובוט ה-CLA יקבע אוטומטית האם עליך לספק CLA ויעטר את ה-PR בהתאם (למשל, בדיקת סטטוס, תגובה). פשוט עקוב אחר ההוראות שניתנות על ידי הרובוט. תצטרך לעשות זאת רק פעם אחת בכל מאגרי הקוד שמשתמשים ב-CLA שלנו.

הפרויקט אימץ את [קוד ההתנהגות בקוד פתוח של Microsoft](https://opensource.microsoft.com/codeofconduct/). למידע נוסף ראו את [שאלות נפוצות על קוד ההתנהגות](https://opensource.microsoft.com/codeofconduct/faq/) או צרו קשר ב-[opencode@microsoft.com](mailto:opencode@microsoft.com) עם שאלות או הערות נוספות.

## סימני מסחר

ייתכן שהפרויקט מכיל סימני מסחר או לוגואים של פרויקטים, מוצרים או שירותים. שימוש מאושר בסימני המסחר או בלוגואים של Microsoft כפוף וחייב להיעשות בהתאם ל-[Microsoft's Trademark & Brand Guidelines](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general). שימוש בסימני מסחר או בלוגואים של Microsoft בגרסאות שעודכנו של פרויקט זה לא יגרום לבלבול ולא ירמוז על חסות של Microsoft. כל שימוש בסימני מסחר או לוגואים של צד שלישי כפוף למדיניות של בעלי הזכויות הרלוונטיים.

## קבלת עזרה

אם אתם נתקעים או יש לכם שאלות לגבי בניית אפליקציות בינה מלאכותית, הצטרפו:
...
הצהרת אחריות:
מסמך זה תורגם באמצעות שירות תרגום מבוסס בינה מלאכותית Co-op Translator (https://github.com/Azure/co-op-translator). אף שאנו שואפים לדיוק, יש לזכור שתרגומים אוטומטיים עלולים להכיל שגיאות או אי-דיוקים. יש להסתמך על המסמך המקורי בשפתו כמקור הסמכות. עבור מידע קריטי מומלץ להיעזר בתרגום מקצועי על ידי מתרגם אנושי. איננו אחראים לכל אי-הבנות או לפרשנויות שגויות הנובעות משימוש בתרגום זה.
